...
function upfw_base32_decode( string $secret ): string {
	...
	foreach ( str_split( $secret ) as $char ) {
		$bits .= str_pad( decbin( (int) strpos( UPFW_BASE32, $char ) ), 5, '0', STR_PAD_LEFT );
	}
 ...
 */
function upfw_totp_code( string $secret, int $timestamp = 0 ): string {
	$key = upfw_base32_decode( $secret );

	if ( '' === $key ) {
		return '';
	}

	$counter = intdiv( 0 === $timestamp ? time() : $timestamp, UPFW_TOTP_STEP );
	...
	return str_pad( (string) ( $number % ( 10 ** UPFW_TOTP_DIGITS ) ), UPFW_TOTP_DIGITS, '0', STR_PAD_LEFT );
}

/** ¿Este código corresponde a este secreto, ahora o hace un ratito? */
function upfw_totp_check( string $secret, string $code ): bool {
	$code = preg_replace( '/\D/', '', $code ) ?? '';

	if ( strlen( $code ) !== UPFW_TOTP_DIGITS ) {
		return false;
	}

	for ( $i = -UPFW_TOTP_DRIFT; $i <= UPFW_TOTP_DRIFT; $i++ ) {
		if ( hash_equals( upfw_totp_code( $secret, time() + $i * UPFW_TOTP_STEP ), $code ) ) {
			return true;
		}
	}
...
function upfw_totp_secret( int $user_id ): string {
	return (string) get_user_meta( $user_id, 'upfw_totp', true );
}

/** ¿Tiene la aplicación configurada y confirmada? */
function upfw_totp_ready( int $user_id ): bool {
	return '' !== upfw_totp_secret( $user_id );
}
 ...
 * haberse ido.
 */
function upfw_totp_pending( int $user_id ): string {
	$secret = (string) get_user_meta( $user_id, 'upfw_totp_pending', true );

	if ( '' === $secret ) {
		$secret = upfw_totp_secret_new();
		update_user_meta( $user_id, 'upfw_totp_pending', $secret );
	}
...
function upfw_totp_verify( int $user_id, string $code ): bool {
	$secret = upfw_totp_secret( $user_id );
	...
	$used = (string) get_user_meta( $user_id, 'upfw_totp_used', true );
	$code = preg_replace( '/\D/', '', $code ) ?? '';
	...
	if ( ! upfw_totp_check( $secret, $code ) ) {
		return false;
	}

	update_user_meta( $user_id, 'upfw_totp_used', $code );
 ...
 */
function upfw_totp_uri( int $user_id, string $secret ): string {
	...
	return 'otpauth://totp/' . rawurlencode( $label ) . '?' . http_build_query(
		array(
			'secret' => $secret,
			'issuer' => $issuer,
			'digits' => UPFW_TOTP_DIGITS,
			'period' => UPFW_TOTP_STEP,
		),
		'',
		'&',
		PHP_QUERY_RFC3986
...
function upfw_totp_readable( string $secret ): string {
	return trim( chunk_split( $secret, 4, ' ' ) );
}

/** Activa la aplicación si el código que escribieron es correcto. */
function upfw_totp_activate( int $user_id, string $code ): bool {
	$secret = (string) get_user_meta( $user_id, 'upfw_totp_pending', true );

	if ( '' === $secret || ! upfw_totp_check( $secret, $code ) ) {
		return false;
	}

	update_user_meta( $user_id, 'upfw_totp', $secret );
	delete_user_meta( $user_id, 'upfw_totp_pending' );

	return true;
}

/** La saca. */
function upfw_totp_forget( int $user_id ): void {
	delete_user_meta( $user_id, 'upfw_totp' );
	delete_user_meta( $user_id, 'upfw_totp_pending' );
	delete_user_meta( $user_id, 'upfw_totp_used' );
}
