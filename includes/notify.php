 ...
 */
function upfw_default_notifications(): array {
	$prefs = array();

	$prefs['upfw_notify_login'] = array(
		'label'   => __( 'When somebody signs in to my account from a new device', 'users-plus-for-wordpress' ),
		'help'    => __( 'The first time a browser or a phone gets in. From then on, that one is quiet.', 'users-plus-for-wordpress' ),
		'default' => '1',
	);

	$prefs['upfw_notify_security'] = array(
		'label'   => __( 'When something in my security changes', 'users-plus-for-wordpress' ),
		'help'    => __( 'A passkey added or removed, two-step verification turned on or off, a social account linked or unlinked.', 'users-plus-for-wordpress' ),
		'default' => '1',
	);
...
function upfw_wants( int $user_id, string $key ): bool {
 ...
 */
function upfw_notify( int $user_id, string $key, string $subject, string $body ): bool {
	if ( ! upfw_wants( $user_id, $key ) ) {
		return false;
	}
...
function upfw_site_name(): string {
 ...
 */
function upfw_device_id(): string {
 ...
 *
 * Se cuelga de `upfw_logged_in`, que es por donde pasan todas las formas de
 * entrar que maneja el plugin: el enlace, la contraseña, una red social y una
 * passkey. Una sola vez por aparato.
 */
function upfw_notify_new_device( int $user_id, string $via ): void {
	$id        = upfw_device_id();
	$conocidos = (array) get_user_meta( $user_id, 'upfw_devices', true );
	...
	$conocidos[] = $id;
	update_user_meta( $user_id, 'upfw_devices', array_slice( $conocidos, -20 ) );
	...
	$agente = upfw_user_agent( isset( $_SERVER['HTTP_USER_AGENT'] ) ? sanitize_text_field( wp_unslash( $_SERVER['HTTP_USER_AGENT'] ) ) : '' );
	...
	upfw_notify(
		...
		sprintf(
			...
			__( 'New sign-in to your account on %s', 'users-plus-for-wordpress' ),
			upfw_site_name()
		),
		sprintf(
			/* translators: 1: aparato y navegador, 2: fecha y hora, 3: cómo entró, 4: dirección del área de cuenta */
			__( "Somebody just signed in to your account.\n\n%1\$s\n%2\$s\nWay in: %3\$s\n\nIf it was you, there is nothing to do. If it was not, close that session and review your security here:\n%4\$s", 'users-plus-for-wordpress' ),
			$aparato,
			wp_date( 'j M Y, H:i' ),
			upfw_via_label( $via ),
			upfw_account_url( 'security' )
		)
	);
}
add_action( 'upfw_logged_in', 'upfw_notify_new_device', 10, 2 );

/** Cómo se entró, en castellano. */
function upfw_via_label( string $via ): string {
	$labels = array(
		'link'     => __( 'a link sent to your email', 'users-plus-for-wordpress' ),
		'password' => __( 'your password', 'users-plus-for-wordpress' ),
		'sso'      => __( 'a social account', 'users-plus-for-wordpress' ),
		'passkey'  => __( 'a passkey', 'users-plus-for-wordpress' ),
	);
