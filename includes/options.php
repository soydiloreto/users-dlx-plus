 ...
 */
function users_plus_option_defaults(): array {
	return array(
		...
		'users_plus_login_method'       => 'both',
		// Página con el formulario de acceso (el shortcode [users_plus_login] o el
		// que ponga el sitio). En 0 se usa wp-login.php.
		'users_plus_login_page'         => 0,
		// Minutos que vale el enlace del correo.
		'users_plus_login_expiry'       => 15,
		// Segundos entre dos pedidos para el mismo correo.
		'users_plus_login_throttle'     => 60,
		// Crear la cuenta si el correo no existe. Apagado, el enlace sólo
		// sirve para quien ya está registrado.
		'users_plus_login_register'     => 1,
		// Rol de las cuentas creadas así.
		'users_plus_login_role'         => 'subscriber',
		'users_plus_login_subject'      => '',
		'users_plus_login_body'         => '',

		// ── Duración de la sesión ─────────────────────────────────────
		'users_plus_session_long_days'  => 30,  // Con "recordarme".
		'users_plus_session_short_days' => 2,   // Sin "recordarme".
		...
		'users_plus_account_sections'   => array(),
		...
		'users_plus_handle_cooldown'    => 30,
		'users_plus_handle_reserved'    => '',

		// ── La foto de perfil ─────────────────────────────────────────
		// Dejar que la persona suba la suya.
		'users_plus_avatar_upload'      => 1,
		...
		'users_plus_2fa_remember_days'  => 30,
		...
		'users_plus_privacy_export'     => 1,
		'users_plus_privacy_delete'     => 1,

		// ── Sesiones ────────────────────────────────────────────────
		// Si cada persona ve dónde tiene la sesión abierta y puede cerrarlas.
		'users_plus_sessions_show'      => 1,

		'users_plus_passkey_enabled'    => 0,
		...
		'users_plus_wp_registration'    => 'site',
 ...
 * @return mixed
 */
function users_plus_option( string $key, $fallback = null ) {
	$defaults = users_plus_option_defaults();
	 ...
	 */
	return apply_filters( 'users_plus_option', $value, $key );
}

/**
 * ¿Este ajuste lo está forzando el sitio desde código?
 *
 * Un plugin como cst-core puede fijar un valor por el filtro `users_plus_option`
 ...
 */
function users_plus_option_forced( string $key ): bool {
	$defaults = users_plus_option_defaults();
	$stored   = get_option( $key, null );
	$stored   = null === $stored ? ( $defaults[ $key ] ?? null ) : $stored;

	return users_plus_option( $key ) !== $stored;
}
 ...
 */
function users_plus_option_forced_by(): array {
	global $wp_filter;

	if ( ! isset( $wp_filter['users_plus_option'] ) ) {
		return array();
	}

	$quienes = array();

	foreach ( $wp_filter['users_plus_option']->callbacks as $enganchados ) {
...
function users_plus_login_expiry(): int {
	return max( 1, min( 1440, (int) users_plus_option( 'users_plus_login_expiry' ) ) );
}
 ...
 */
function users_plus_login_url(): string {
	$id  = (int) users_plus_option( 'users_plus_login_page' );
	 ...
	 * @param string $url
	 */
	return apply_filters( 'users_plus_login_url', $url );
}
 ...
 */
function users_plus_save_options( array $input ): void {
	$defaults = users_plus_option_defaults();
