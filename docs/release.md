...
For maintainers. End users get the plugin from [wordpress.org/plugins/users-dlx-plus](https://wordpress.org/plugins/users-dlx-plus/) — they don't need to read this document.
...
The PHP `Version:` header in `users-dlx-plus.php` and the `USERS_DLX_PLUS_VERSION` constant carry a `-dev` suffix on `main` between releases. The `Stable tag:` in `readme.txt` does **not** — it always holds the last published release, or before any release the next intended one.

| State | PHP `Version:` | `USERS_DLX_PLUS_VERSION` | readme `Stable tag:` |
| --- | --- | --- | --- |
...
2. **Open a release-prep PR.** Branch name: `chore/release-X.Y.Z`. The PR does three things:
   - Drops the `-dev` suffix in `users-dlx-plus.php` (the `Version:` header **and** the `USERS_DLX_PLUS_VERSION` constant).
...
6. **The deploy workflow takes it from there.** [`.github/workflows/deploy.yml`](../.github/workflows/deploy.yml) fires on tag push; it does:
   ...
   - Strict version-alignment of all three markers — PHP `Version:`, `USERS_DLX_PLUS_VERSION`, readme `Stable tag:` — against the git tag.
...
7. **Verify on wp.org** within ~10 minutes. The new version should appear at `https://wordpress.org/plugins/users-dlx-plus/`. wp.org does not run automated rollouts — sites with auto-update enabled pick it up over the next ~12 hours via the WordPress core update check.

8. **Bump back to dev.** Open a follow-up PR `chore/bump-(X.Y.Z+1)-dev` that:
   - Sets PHP `Version:` and `USERS_DLX_PLUS_VERSION` to `(next intended version)-dev`.
