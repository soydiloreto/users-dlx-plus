...
```bash
git clone https://github.com/soydiloreto/users-plus-for-wordpress.git
cd users-plus-for-wordpress
make install     # composer install — pulls dev tooling into vendor/
make env         # boots wp-env at http://localhost:8888
```

When `make env` finishes, open <http://localhost:8888>. Log in with `admin` / `password`. The plugin is already mounted at `wp-content/plugins/users-plus-for-wordpress/` — activate it from the **Plugins** screen.
...
| `make i18n` | `wp i18n make-pot` — extract the translatable strings into `build/users-plus-for-wordpress.pot`. |
...
If you'd rather use your own WordPress setup instead of `wp-env`, clone this repo directly into `wp-content/plugins/users-plus-for-wordpress/` of your existing WordPress install. The plugin has no build step — it runs straight from source.
