...
Open a [new issue](https://github.com/soydiloreto/users-dlx-plus/issues/new/choose) and pick the appropriate template:
...
For **end-user support questions** (how do I configure this?, my upload is not working, etc.) please use the [wp.org support forum](https://wordpress.org/support/plugin/users-dlx-plus/) instead — that's where most users look for answers and where we maintain a public Q&A.
...
- **All user-facing strings** must be wrapped in WordPress translation functions (`__()`, `_e()`, `_n()`, etc.) with the text domain `users-dlx-plus`. `sprintf()` placeholders need a `/* translators: */` comment **on the line immediately preceding** the translation call (a blank line in between makes the comment invisible to gettext).
...
The `Version:` header in `users-dlx-plus.php` (and the
...
2. The maintainer drops the `-dev` suffix in `users-dlx-plus.php`
