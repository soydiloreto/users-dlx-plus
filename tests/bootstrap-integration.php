 ...
 * - This plugin is mounted at /var/www/html/wp-content/plugins/users-dlx-plus/
...
if (!defined('USERS_DLX_PLUS_DIR')) {
    fwrite(STDERR, "ERROR: users-dlx-plus is not activated in the tests environment.\n");
    fwrite(STDERR, "Run: npx wp-env run tests-cli wp plugin activate users-dlx-plus\n");
    exit(1);
}
...
if (!defined('USERS_DLX_PLUS_INTEGRATION_TESTS')) {
    define('USERS_DLX_PLUS_INTEGRATION_TESTS', true);
}
...
$_users_dlx_plus_wp_die_test_handler = function ($message, $title = '', $args = []) {
...
add_filter('wp_die_ajax_handler', function () use ($_users_dlx_plus_wp_die_test_handler) {
    return $_users_dlx_plus_wp_die_test_handler;
}, 999);

add_filter('wp_die_handler', function () use ($_users_dlx_plus_wp_die_test_handler) {
    return $_users_dlx_plus_wp_die_test_handler;
}, 999);
...
echo "  Plugin ver:  " . USERS_DLX_PLUS_VERSION . "\n";
