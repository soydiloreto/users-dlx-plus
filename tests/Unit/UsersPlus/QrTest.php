...
class QrTest extends TestCase {
	...
	protected function setUp(): void {
		parent::setUp();
		require_once USERS_PLUS_DIR . 'includes/qr.php';
	}

	public function test_la_version_crece_con_el_texto(): void {
		// El lado es 17 + 4 × versión.
		$this->assertCount( 21, users_plus_qr_matrix( 'hola' ) );
		$this->assertCount( 45, users_plus_qr_matrix( str_repeat( 'a', 150 ) ) );
		$this->assertCount( 57, users_plus_qr_matrix( str_repeat( 'a', 260 ) ) );
	}

	public function test_lo_que_no_entra_devuelve_null(): void {
		$this->assertNull( users_plus_qr_matrix( str_repeat( 'a', 500 ) ) );
	}

	public function test_los_tres_patrones_de_posicion_estan(): void {
		$m    = users_plus_qr_matrix( 'otpauth://totp/x?secret=ABCDEFGHIJKLMNOP' );
		$lado = count( $m );
	...
	public function test_el_svg_trae_el_margen_que_pide_el_estandar(): void {
		$svg = users_plus_qr_svg( 'hola', 200 );
	...
	public function test_sin_espacio_no_devuelve_svg(): void {
		$this->assertSame( '', users_plus_qr_svg( str_repeat( 'a', 500 ) ) );
	}
}
