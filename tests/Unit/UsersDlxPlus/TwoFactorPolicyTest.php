...
class TwoFactorPolicyTest extends TestCase {
	...
	protected function setUp(): void {
		...
		require_once USERS_DLX_PLUS_DIR . 'includes/auth-email.php';
		require_once USERS_DLX_PLUS_DIR . 'includes/auth.php';
		require_once USERS_DLX_PLUS_DIR . 'includes/login.php';
	...
	private function ajustes( array $valores ): void {
		$base = array(
			'users_dlx_plus_2fa_mode'          => 'required',
			'users_dlx_plus_2fa_methods'       => array( 'email' ),
			'users_dlx_plus_2fa_roles'         => array(),
			'users_dlx_plus_2fa_link'          => 'auto',
			'users_dlx_plus_2fa_remember_days' => 0,
		);
	...
	public function test_apagado_no_pide_nunca(): void {
		$this->ajustes( array( 'users_dlx_plus_2fa_mode' => 'off' ) );

		$this->assertFalse( users_dlx_plus_2fa_required( self::USUARIO, 'password' ) );
	}

	public function test_obligatorio_pide_a_todos(): void {
		$this->assertTrue( users_dlx_plus_2fa_required( self::USUARIO, 'password' ) );
	}
	...
	public function test_sin_metodos_no_puede_pedir_nada(): void {
		...
		$this->ajustes( array( 'users_dlx_plus_2fa_methods' => array() ) );
	...
	public function test_con_el_correo_como_unico_segundo_paso_el_enlace_no_lo_pide(): void {
		...
		$this->assertFalse( users_dlx_plus_2fa_required( self::USUARIO, 'link' ) );
	}

	public function test_con_una_aplicacion_disponible_el_enlace_si_lo_pide(): void {
		$this->ajustes( array( 'users_dlx_plus_2fa_methods' => array( 'email', 'totp' ) ) );
		update_user_meta( self::USUARIO, 'users_dlx_plus_totp', 'GEZDGNBVGY3TQOJQGEZDGNBVGY3TQOJQ' );

		$this->assertTrue( users_dlx_plus_2fa_required( self::USUARIO, 'link' ) );
	}

	public function test_el_sitio_puede_forzar_las_dos_respuestas(): void {
		$this->ajustes( array( 'users_dlx_plus_2fa_link' => 'always' ) );
		$this->assertTrue( users_dlx_plus_2fa_required( self::USUARIO, 'link' ) );

		$this->ajustes( array( 'users_dlx_plus_2fa_link' => 'never', 'users_dlx_plus_2fa_methods' => array( 'email', 'totp' ) ) );
		update_user_meta( self::USUARIO, 'users_dlx_plus_totp', 'GEZDGNBVGY3TQOJQGEZDGNBVGY3TQOJQ' );
		$this->assertFalse( users_dlx_plus_2fa_required( self::USUARIO, 'link' ) );
	}

	public function test_con_roles_elegidos_solo_a_esos(): void {
		$this->ajustes( array( 'users_dlx_plus_2fa_roles' => array( 'administrator' ) ) );
	...
	public function test_opcional_solo_a_quien_lo_prendio(): void {
		$this->ajustes( array( 'users_dlx_plus_2fa_mode' => 'optional' ) );

		$this->assertFalse( users_dlx_plus_2fa_required( self::USUARIO, 'password' ) );

		update_user_meta( self::USUARIO, 'users_dlx_plus_2fa_on', 1 );
		$this->assertTrue( users_dlx_plus_2fa_required( self::USUARIO, 'password' ) );
	}
	...
	public function test_con_el_enlace_como_unica_puerta_no_se_pide_en_ninguna(): void {
		...
		$this->ajustes( array( 'users_dlx_plus_login_method' => 'link' ) );

		$this->assertSame( array( 'link' ), array_keys( users_dlx_plus_2fa_ways( self::USUARIO ) ) );
		$this->assertSame( array(), users_dlx_plus_2fa_ways_asked( self::USUARIO ) );
	}

	public function test_con_contrasena_prendida_el_enlace_se_saltea_pero_la_contrasena_no(): void {
		$this->ajustes( array( 'users_dlx_plus_login_method' => 'both' ) );

		$vias = users_dlx_plus_2fa_ways( self::USUARIO );

		$this->assertFalse( $vias['link']['asked'] );
		$this->assertTrue( $vias['password']['asked'] );
		$this->assertSame( array( 'your password' ), users_dlx_plus_2fa_ways_asked( self::USUARIO ) );
	}

	public function test_sin_dias_de_memoria_ningun_navegador_es_de_confianza(): void {
		$this->assertFalse( users_dlx_plus_2fa_trusted( self::USUARIO ) );
	}

	public function test_los_codigos_de_respaldo_no_se_guardan_en_claro(): void {
		$codigos = users_dlx_plus_backup_generate( self::USUARIO, 4 );

		$this->assertCount( 4, $codigos );
		$this->assertSame( 4, users_dlx_plus_backup_left( self::USUARIO ) );

		foreach ( $GLOBALS['cst_test_user_meta'][ self::USUARIO ]['users_dlx_plus_backup_codes'] as $guardado ) {
			$this->assertNotContains( $guardado, $codigos );
		}
	}

	public function test_un_codigo_de_respaldo_sirve_una_sola_vez(): void {
		$codigos = users_dlx_plus_backup_generate( self::USUARIO, 3 );

		$this->assertTrue( users_dlx_plus_backup_use( self::USUARIO, $codigos[1] ) );
		$this->assertSame( 2, users_dlx_plus_backup_left( self::USUARIO ) );
		$this->assertFalse( users_dlx_plus_backup_use( self::USUARIO, $codigos[1] ) );
	}

	public function test_un_codigo_de_respaldo_inventado_no_sirve(): void {
		users_dlx_plus_backup_generate( self::USUARIO, 3 );

		$this->assertFalse( users_dlx_plus_backup_use( self::USUARIO, 'noexiste00' ) );
		$this->assertSame( 3, users_dlx_plus_backup_left( self::USUARIO ) );
	}
}
