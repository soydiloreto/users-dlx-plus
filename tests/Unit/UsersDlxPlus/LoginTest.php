...
class LoginTest extends TestCase {
	...
	public function test_un_token_recien_creado_es_valido(): void {
		$token = users_dlx_plus_token_create( self::USUARIO );

		$this->assertTrue( users_dlx_plus_token_valid( self::USUARIO, $token ) );
	}
	...
	public function test_el_token_no_se_guarda_en_claro(): void {
		...
		$this->assertNotSame(
			$token,
			$GLOBALS['cst_test_user_meta'][ self::USUARIO ][ USERS_DLX_PLUS_META_HASH ],
			'En la base tiene que quedar el hash, nunca el token'
		);
	}

	public function test_un_token_ajeno_no_sirve(): void {
		users_dlx_plus_token_create( self::USUARIO );

		$this->assertFalse( users_dlx_plus_token_valid( self::USUARIO, 'token-inventado' ) );
	}

	public function test_el_token_de_otra_persona_no_sirve(): void {
		$mio = users_dlx_plus_token_create( self::USUARIO );
		users_dlx_plus_token_create( 9999 );

		$this->assertFalse(
			users_dlx_plus_token_valid( 9999, $mio ),
	...
	public function test_el_token_vence(): void {
		...
		$GLOBALS['cst_test_user_meta'][ self::USUARIO ][ USERS_DLX_PLUS_META_EXPIRES ] = time() - 1;

		$this->assertFalse( users_dlx_plus_token_valid( self::USUARIO, $token ) );
	}
	...
	public function test_el_token_sirve_una_sola_vez(): void {
		...
		users_dlx_plus_token_burn( self::USUARIO );

		$this->assertFalse(
			users_dlx_plus_token_valid( self::USUARIO, $token ),
	...
	public function test_sin_token_guardado_nada_valida(): void {
		...
		$this->assertFalse( users_dlx_plus_token_valid( self::USUARIO, 'cualquier-cosa' ) );
	}

	public function test_cada_token_es_distinto(): void {
		$a = users_dlx_plus_token_create( self::USUARIO );
		$b = users_dlx_plus_token_create( self::USUARIO );

		$this->assertNotSame( $a, $b );
		$this->assertFalse(
			users_dlx_plus_token_valid( self::USUARIO, $a ),
